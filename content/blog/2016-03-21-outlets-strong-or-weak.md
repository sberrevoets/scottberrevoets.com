...
While at first this seems like a solid approach, at Lyft we quickly realized we
...
exact syntax, just like any other variable.

For example, if there is a code path that removes an outlet from its superview,
or the outlet is (intentionally) not hooked up in the storyboard, it needs to be
an optional because the outlet is not guaranteed to be there when it's accessed.

`@IBOutlet private var someLabel: UILabel?`

If there is no code path that re-adds the outlet to the view hierarchy, it would
also be good to make it `weak` to not hold on to it unnecessarily when it gets
removed:
 
`@IBOutlet private weak var someLabel: UILabel?`

This ensures that if the label is removed from the superview, it's not being
...
implicitly unwrapped optional is appropriate:

`@IBOutlet private var someLabel: UILabel!`

The outlet isn't `weak` in case the code ever changes so that there *is* a code
...
1. `!` needs a guarantee that the view exists, so always use `strong` to provide
   that guarantee
2. If it's possible the view isn't part of the view hierarchy, use `?` and
   appropriate optional-handling (optional binding/chaining) for safety
3. If you don't need a view anymore after removing it from the view hierarchy,
   use `weak` so it gets removed from memory.

Applying these three rules means you properly use the optional semantics. After
all, using `!` for a view that may not exist is no different than defining any
other property as an implicitly unwrapped optional that may not exist.
